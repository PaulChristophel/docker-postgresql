...
ARG BASE=docker.io/opensuse/tumbleweed:latest
ARG IMAGE_TITLE="CloudNativePG PostgreSQL on OpenSUSE"
ARG IMAGE_DESCRIPTION="PostgreSQL built from upstream source on OpenSUSE for CloudNativePG."
...
RUN build_packages="\
 ...
 && zypper --non-interactive install --no-recommends ${build_packages} \
...
RUN runtime_packages="\
 ...
 && if [ "${PG_MAJOR}" -ge 18 ]; then \
      runtime_packages="${runtime_packages} libcurl4 liburing2"; \
    fi \
